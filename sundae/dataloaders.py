...
def tokenize_wmt_example(example, en_tokenizer, de_tokenizer, add_special_tokens=True):
    ...
    example["source"] = en_tokenizer.encode(example["source"], add_special_tokens=add_special_tokens)
    example["target"] = de_tokenizer.encode(example["target"], add_special_tokens=add_special_tokens)
    return example
...
def get_dataloaders(config):
    ...
    if config.data.name.lower() == "text8":
    ...
    elif config.data.name.lower() == "wmt14":
        ...
        for split in ["train", "validation"]:
            data[split] = data[split].map(
                lambda x: tokenize_wmt_example(x, en_tokenizer, de_tokenizer),
